...
$("#wirtereview").click(function () {
				...
				var popup_num = $("#popup_nums").val();
				var member_num = $("#member_nums").val();
				 console.log("popup_num:"+popup_num);
				 console.log("member_num:"+member_num);
				$.ajax({
					type:"get",
					dataType:"json",
					url:"/review/memberpopupreviewcheck",
					data:{"popup_num": popup_num,
						  "member_num": member_num},
					success:function(res){
						if(res.previewcount==0){ 
							 location.href="/popup/popupreviewform?popup_num="+popup_num; 
						}else{
...
<body>
	...
	<input type="hidden" id="popup_nums" value="${popup_num }">
	<c:set var="loginok" value="${sessionScope.loginok}"></c:set>
	<div class="popup_info">

		<b>${Edto.popup_subject }</b>
		<hr>
		<br>

		<div class="popup_detailimg">
			<c:if test="${Edto.popup_image!=null}">
				<img alt="popup_img" src="${Edto.popup_image}"
		...
		<div class="popup_detailcontent">
			${Edto.popup_content}<br>
		</div>
		...
		<div class="popup_data">
			...
			<div class="like_popup">
				<c:if test="${not empty loginok}">
					<c:if test="${like_check eq 1}">
						<a class="icon heart active" popup_num="${Edto.popup_num}"> <img
					...
					<c:if test="${like_check ne 1}">
						<a class="icon heart" popup_num="${Edto.popup_num}"> <img
							src="../../image/heart.png" alt="좋아요" style="width: 20px;">
						</a>
						<span class="Likecount">${Like}</span>
			...
			<br>
			<ul>
				<li><b>제목:</b> ${Edto.popup_subject }</li>
				<li><b>기간:</b> ${Edto.popup_term }</li>
				<li><b>장소:</b> ${Edto.popup_location}</li>
				<li><b>출처:</b> <a style="color: gray;"
					href="https://www.popply.co.kr${Edto.popup_link}"> 사이트 바로가기</a></li>
		...
		<div
			...
			<a
				style="color: black; font-size: 15px; position: absolute; margin-top: 5px;"
				href="https://map.kakao.com/link/to/${Edto.popup_location},${Edto.popup_latitude},${Edto.popup_longitude}"><strong>길찾기</strong></a>
			<br> <small style="font-size: 10px; color: gray;">${Edto.popup_detaillocation}</small>
		</div>
		<script type="text/javascript"
...
mapOption = { 
    center: new kakao.maps.LatLng(${Edto.popup_latitude},${Edto.popup_longitude}), // 지도의 중심좌표
...
//마커가 표시될 위치입니다 
var markerPosition  = new kakao.maps.LatLng(${Edto.popup_latitude},${Edto.popup_longitude}); 
...
map.addControl(zoomControl, kakao.maps.ControlPosition.RIGHT);
	...
	<br>
		...
		<b>후기(${popupreviewcount})</b>
		 <b style="font-size: 14px;"> ★${avgscore} ${noreview }</b>
		...
		<div class="lists">
			...
			<c:if test="${noreview!=0}">
			<a href="/review/popupreviewlist?popup_num=${Edto.popup_num}"
		...
		<div id="reveiw-list">
			...
			<br>
			<c:forEach var="e" items="${mlist}" varStatus="i">
			<input type="hidden" id="popup_num" value="${e.popup_num }">
				...
				<div class="reviewbox">
					...
					<c:if test="${member_num eq e.member_num}">
							 ...
							 <img class="rmenubtn" src="../../image/menuimage2.png" style="width: 25px; height: 15px;"   >
						...
						<b >${e.review_score }</b> 
						<b style="float: left; margin-top: 12px; margin-right: 2px; font-size: 7px;">${e.review_writeday }</b>
					</div>
						<br>
						<div class="reviewinfo">
						
						<b style="word-break:break-all; margin-left: 10px;" >${e.review_content }</b>
						</div>
		...
		<div class="writebtn">
				...
				<c:if test="${empty loginok}">
				<button id="noreview"><b>후기 작성</b></button>
